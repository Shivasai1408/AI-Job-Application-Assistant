...
async function loadResumes() {
  ...
  container.querySelectorAll('.download-resume').forEach(function(btn) {
    btn.addEventListener('click', async function() {
      const id = this.dataset.id;
      const result = await downloadsAPI.downloadResume(id);
      if (result.error || !result.data) {
        showError(container, result.error || 'Failed to download resume');
        return;
      }

      const resume = resumes.find(function(item) { return String(item.id) === String(id); });
      const fileName = `${(resume?.title || 'resume').replace(/[^a-z0-9]+/gi, '-').toLowerCase()}.txt`;
...
window.initResumeBuilder = function() {
  const saveMessage = document.getElementById('resume-save-message');
  ...
  form.parentNode.replaceChild(newForm, form);
  newForm.reset();
  document.querySelectorAll('#page-resume-builder input, #page-resume-builder textarea').forEach(function(input) {
    input.value = '';
  });

  newForm.addEventListener('submit', async function(e) {
    e.preventDefault();
    const builder = document.querySelector('#page-resume-builder');
    const inputs = builder.querySelectorAll('input, textarea');
    const values = Array.from(inputs).map(function(input) {
      return input.value.trim();
    });
    const name = values[0] || 'Untitled';
    const title = values[1] ? name + ' - ' + values[1] : name + ' Resume';
    const content = [
      name,
      values[2] ? 'Email: ' + values[2] : '',
      values[3] ? 'Phone: ' + values[3] : '',
      values[4] ? 'Location: ' + values[4] : '',
      values[5] ? 'LinkedIn: ' + values[5] : '',
    ...
    if (result.error) {
      ...
      message.id = 'resume-save-message';
...
window.initATSScore = function() {
  ...
  newForm.addEventListener('submit', async function(e) {
    ...
    if (resultsDiv) {
      ...
      const scoreElement = resultsDiv.querySelector('.stat-card-value');
